...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription('Remove user from bypass from protection system.')
    ...
    async run(client, interaction, language, reply, replyEmbeds, name) {
        try {
            ...
            if(!check.includes(type.toLowerCase())){
              return interaction.reply({ content: reply.Protection.Reply4.replace("[USER]", member).replace("[ACTION]", type.toLowerCase()), allowedMentions: { repliedUser: false } })
            }
            
            db.pull("whiteList_" + interaction.guild.id + "_" + member.id + "_" + client.user.id, type.toLowerCase()).then(() =>{
              return interaction.reply({ content: reply.Protection.Reply5.replace("[ACTION]", type.toLowerCase()).replace("[USER]", member) , allowedMentions: { repliedUser: false } })
            })
        } catch (error) {
            console.log(error)
