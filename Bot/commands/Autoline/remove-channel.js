    ...
    run: async (client, message, args, language, reply, replyEmbeds, name) => {
        try {
            ...
            if (channels.includes(channel.id)) {
                db.pull("Autoline_" + message.guild.id + "_" + client.user.id, channel.id).then(() => {
                    return message.reply({ content: reply.Autoline.Reply4.replace("[CHANNEL]", channel), allowedMentions: { repliedUser: false } })
                })
            }else{
                return message.reply({content: reply.Autoline.Reply5.replace("[CHANNEL]", channel), allowedMentions: {repliedUser: false}})
            }
        } catch (error) {
            console.log(error)
