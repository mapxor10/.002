...
*/
    run: async (client, message, args, language, reply, replyEmbeds, name) => {
        try {
            ...
            if(channels.includes(channel.id)) return message.reply({content: reply.Suggestion.Reply2.replace("[CHANNEL]", channel), allowedMentions: {repliedUser: false}})
            db.push("Suggestion_" + message.guild.id + "_" + client.user.id, channel.id).then(() =>{
                return message.reply({content: reply.Suggestion.Reply3.replace("[CHANNEL]", channel), allowedMentions: {repliedUser: false}})
        })
        } catch (error) {
            console.log(error)
