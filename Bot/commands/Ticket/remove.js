...
*/
    run: async (client, message, args, language, reply, replyEmbeds, name) => {
        try {
            ...
            if(member.id == message.channel.topic || member.id == ticketData.owner){
                const embed = new EmbedBuilder()
                    .setColor('Red')
                    .setDescription(reply.Ticket.Reply4)
                return message.reply({ embeds: [embed], ephemeral: true, allowedMentions: { repliedUser: false }})
            }
            message.channel.permissionOverwrites.edit(member.id, {
                ...
                const embed = new EmbedBuilder()
                .setColor('Green')
                .setDescription(reply.Ticket.Reply5.replace("[USER]", member).replace("[ROOM]", message.channel))
