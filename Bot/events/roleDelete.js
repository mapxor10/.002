...
module.exports.run = async (client, role) => {
  ...
  const commandsdb = new Database(`/Json-Database/CommandsData/BotsCommands_${client.user.id}.json`);
    ...
    let log = logdb.get("role-deleted_" + role.guild?.id + "_" + client.user.id) || null
    ...
    if(log){
      ...
      role.guild.fetchAuditLogs({ action: 32, limit: 1 }).then(async (audit) => {
        ...
        .setTimestamp()
        .setTitle(reply.Log.Reply10.replace("[ROLE]", role.name))
        .addFields(
    ...
    if(hasProtection){
      role.guild.fetchAuditLogs({ action: 32, limit: 1 }).then(async (audit) => {
        ...
        if (info.action == 32 && info.target?.id == role.user?.id && info.executor) {
          ...
          let limit = protectiondb.get("actionLimit_" + "delete_roles_" + role.guild?.id + "_" + client.user.id) || 5
          let automode = protectiondb.get("autoMode_" + "delete_roles_" + role.guild?.id + "_" + client.user.id) || "delete_roles"
          ...
          if (whitelistCheck && (whitelistCheck.includes("delete_roles") || whitelistCheck.includes("all"))) {
            if(logID){
              ...
              if(log){
                ...
                .setFooter({ iconURL: info.executor.avatarURL({ dynamic: true }), text: info.executor.username })
                .setTimestamp()
                .setDescription(reply.Protection.Reply10.replace("[ROLE]", role.name).replace("[ACTION]", "deleted").replace("[AUTHOR]", info.executor).replace("[TIMES]", reply.Protection.Whitelist.yes))
                log.send({embeds: [embed]})
              }
            }
          }else{
            let userData = protectiondb.get("protectionData_" + "delete_roles_" + role.guild?.id + "_" + info.executor?.id + "_" + client.user.id) || 0
            ...
            if(logID){
              ...
              if(log){
                ...
                .setFooter({ iconURL: info.executor.avatarURL({ dynamic: true }), text: info.executor.username })
                .setTimestamp()
                .setDescription(reply.Protection.Reply12.replace("[ROLE]", role.name).replace("[ACTION]", "deleted").replace("[AUTHOR]", info.executor).replace("[TIMES]", timeLeft))
                log.send({embeds: [embed]})
              }
            }
            const endTime = moment().add(20, 'minutes').format('YYYY-MM-DD HH:mm:ss');
            protectiondb.set("protectionData_" + "delete_roles_" + role.guild?.id + "_" + info.executor?.id + "_" + client.user.id, userData + 1).then(() => {
              ...
              setTimeout(() =>{
                if(serverOwner && done == true){
                  ...
                  .addFields(
                    ...
                    {name: reply.Protection.OwnerMSG.Fileds.Field4.name , value: reply.Protection.OwnerMSG.Fileds.Field4.value.replace("[ACTION]", "Deleting roles")},
                  )
