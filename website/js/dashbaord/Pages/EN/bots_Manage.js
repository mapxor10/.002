function renewFuncation(button) {
    ...
    createAlertCard('Renew', 'Renew bot subscription', `<span id='renew_price_content'>${price} Coins</span>`, "<input class='comp-range1' type='range' name='' id='renew_input' maxlength='6' value='1' step='1' min='1' max='6'> <span class='c-w range_value' id='renew_months_content'>1 Months</span>", `<button data-bot='${bot}' data-name='${name}' class='save' onclick='buyRenewFunction(this , this.dataset.bot, this.dataset.name, this.parentElement.parentElement, this.parentElement.parentElement.parentElement)'>Renew</button>`)
    ...
    rangeInput.addEventListener('input', () => {
        ...
        priceContent.textContent = `${price * rangeInput.value} Coins`
        monthsContent.textContent = `${rangeInput.value} Months`
    })
