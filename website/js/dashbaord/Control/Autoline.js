var AutolineUploader = document.getElementById("line_uploader_input")
function Add_Remove_AutolineChannel(channel) {
    ...
    xhr.open('POST', '/bot/control/autoline', true);
...
AutolineUploader.onchange = function () {
    var line = document.getElementById("line__holder")
    var file = new FileReader();
    file.readAsDataURL(AutolineUploader.files[0])
...
function AutolineModeChanger(btn) {
    let colddownCheck = colddowns.filter(d => d.name == "AutolineModeChanger" && d.bot == botID && d.user == userID)[0]
    if (colddownCheck) {
        return createAlert('warning', 'Warn', 'You do that too fast.!')
    }
    var xhr = new XMLHttpRequest();
    ...
    requestLimit(10000, 'AutolineModeChanger', botID)
xhr.send(data);
