  ...
  run: async (req, res) => {
    ...
    if (deecoded) {
      ...
      if (req.cookies.token) {
        ...
        if (config.Developer.includes(data.user.id)) botsToFetch = botsData[0]?.data || []
        if (config.Developer.includes(data.user.id)) botsToFetch?.filter(t => {
          if (t.Owner == data.user.id) {
            t.owned = "true"
          }
          botsToFetch = botsToFetch
        })
      ...
      res.render(`./website/html/Pages/Dashboard/Manage/bots.ejs`, args);
    } else {
      return res.end(`<script>window.location.href = "/";</script>`);
    }
  }
