...
module.exports = {
    name: `/bot/control/settings/ticket/:id/:id/load`,
    type: "get",
    run: async (req, res) => {
        ...
        if (deecoded) {
            ...
            if (req.cookies.token) {
                ...
                let botConfigData = {}
                let botData = new Database("../Bot/Json-Database/Settings/Ticket.json")
                let ticketLimit = botData.get("ticketsLimit_" + serverId) || 50
                let channelsFiltered = []
                let categorysFiltered = []
                let rolessFiltered = []
                ...
                localBotsData.servers.filter(s => s.id == serverId)[0].channels.forEach(channel => {
                    if (channel.type == 0 || channel.type == "Guild Text") {
                        ...
                        categorysFiltered.push(channel)
                    }
                })
                ...
                botConfigData.categorys = categorysFiltered
                botConfigData.roles = rolessFiltered
                botConfigData.ticketLimit = ticketLimit
            ...
            res.render(`./website/html/Pages/Control/Ticket.ejs`, args);
        } else {
